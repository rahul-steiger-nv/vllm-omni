...
from vllm_omni.diffusion.offloader.flat_storage import FlatModelCPUOffloadMixin
...
class Cosmos3VFMTransformer(FlatModelCPUOffloadMixin, nn.Module):
    ...
    Model-level CPU offload reuses :class:`FlatModelCPUOffloadMixin`: the UND
    ``reasoner`` and GEN ``generator`` pathways are declared as mutually
    ...
    # language model and the generation (GEN) denoising layers.
    _offload_group_specs = {
        "reasoner": ["language_model.layers"],
        "generator": ["gen_layers"],
    }
    # Small submodules used by both phases; kept GPU-resident.  Optional action
    # and audio projections are listed unconditionally and skipped when absent.
    _offload_resident_modules = [
    ...
    # Reasoner/generator pathway-level CPU offload is provided by
    # FlatModelCPUOffloadMixin via the ``_offload_group_specs`` /
    # ``_offload_resident_modules`` declarations above; ``enable_model_cpu_offload``,
    # ``disable_model_cpu_offload``, ``_offload_context`` and the ``device``
    # override are inherited from the mixin.
